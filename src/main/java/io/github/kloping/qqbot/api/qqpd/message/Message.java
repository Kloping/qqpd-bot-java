...
public class Message
        implements Sender, DirectSender {
    private String id;
    ...
    public MessageAudited sendDirect(String text, Message message) {
        return sendDirect(new MessagePacket().setContent(text).setReplyId(message.id));
    }
    ...
    public MessageAudited sendDirect(MessagePacket packet) {
        ...
        msg.setMsgId(Message.this.id);
        packet2pre(packet, msg);
        return Resource.dmsBase.send(Message.this.guildId, msg, MAP);
    }
